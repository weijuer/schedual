...
<template>
  <div class="month-picker">
    <w-icon class="icon-btn" size="26" @click="reduceMonth()" title="上一月">
      <ArrowCircleLeft12Regular />
    </w-icon>
    <span class="month"> {{ state.date.year }}年{{ month }}月 </span>
    <w-icon class="icon-btn" size="26" @click="addMonth()" title="下一月">
      <ArrowCircleRight12Regular />
    </w-icon>
    <w-icon class="icon-btn" size="26" @click="thisMonth()" title="当月">
      <CalendarToday24Regular />
    </w-icon>
  </div>
</template>
